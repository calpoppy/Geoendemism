...
for(a in wordList) {
  ...
  theFile <-file(file.path(inputPath, paste0('cch_geodat_', a, '.csv')), "r")
  ## read "EdaphicRestrictions_terms.csv"
  blFile <- read.csv2(file = blPath, header = TRUE, sep = ",") 

  ## get the white list of the key word
  tempWhite <- subset(blFile, efloraSearch == a, select = vars_white_list)
  ...
  tempBlack <- subset(blFile, efloraSearch == a, select = vars_black_list)
  ...
  if (grepl('[[:alpha:]]', whiteList[1]) && grepl('[[:alpha:]]', blackList[1])) {
  # if (blackLen != 0 && whiteLen != 0 && grepl('[[:alpha:]]', blackList)) {
